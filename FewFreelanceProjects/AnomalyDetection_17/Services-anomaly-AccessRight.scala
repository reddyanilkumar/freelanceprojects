...
class AnomalyAccessRight {
  ...
  def getAccessRightSecondTable(): JSONArray = {
    val connection: Connection = Common.getConnection()
    val sql: String = "select TO_NUMBER(\"TotalUserAnomaly\") as \"Total Anomaly\",TO_NUMBER(\"TotalDocumentsAtRisk\") as \"Document at Risk\",TO_NUMBER(\"TotalGroupAnomaly\") as \"Total Active Directory Group at Risk\"  from \"AccessRightAnomaly\" where \"key\" = 'access_anomaly'"
    val query: PreparedStatement = connection.prepareStatement(sql)
    val rst: ResultSet = query.executeQuery()
  ...
  def convertResultSetIntoJSON(resultSet: ResultSet, customColumn: String): JSONArray = {
    ...
    val timeStamps: List[String] = new HbaseReadTest().getTimeStamps("timestamp", "AccessRightAnomaly", "access_anomaly", "Anomaly", "TotalUserAnomaly")
    val totalAnomaly: List[String] = new HbaseReadTest().getTimeStamps("TotalUserAnomaly", "AccessRightAnomaly", "access_anomaly", "Anomaly", "TotalUserAnomaly")
  ...
  def getAccessRightPreviousValues(ts: String, column: String): Long = {
    ...
    //println(ts)
    val sql: String = "select TO_NUMBER(\"" + column + "\") from \"AccessRightAnomaly\" where \"key\" = 'access_anomaly'"
    println(sql)
    val query: PreparedStatement = connection.prepareStatement(sql)
    val rst: ResultSet = query.executeQuery()
    var value: Long = 0
