...
class AnomalyClient {
  ...
  def convertResultSetIntoJSON(resultSet: ResultSet, customColumn: String): JSONArray = {
    val jsonArray: JSONArray = new JSONArray()
    val timeStamps: List[String] = new HbaseReadTest().getTimeStamps("timestamp", "ManageAnomaly", "client_anomaly", "AnomalyClient", "TotalAnomaly")
    val totalAnomaly: List[String] = new HbaseReadTest().getTimeStamps("totalanomaly", "ManageAnomaly", "client_anomaly", "AnomalyClient", "TotalAnomaly")
  ...
  def getGUID(user: String): String = {
    val connection: Connection = Common.getConnection()
    val sql: String = "select \"objectGUID\" from \"USERS\" where \"key\" = ?"
    val query: PreparedStatement = connection.prepareStatement(sql)
  ...
  def getADGroups(user: String): String = {
    val connection: Connection = Common.getConnection()
    val sql: String = "select \"memberOf_1\" from \"USERS\" where \"key\" = ?"
    val query: PreparedStatement = connection.prepareStatement(sql)
    query.setString(1, user)
    val rst: ResultSet = query.executeQuery()
    rst.next()
  ...
  def getClientRepoPreviousValues(ts: String, column: String): Long = {
    ...
    val sql: String = "select TO_NUMBER(\"" + column + "\") from \"ManageAnomaly\" where \"key\" = 'client_anomaly'"
    println(sql)
    val query: PreparedStatement = connection.prepareStatement(sql)
    val rst: ResultSet = query.executeQuery()
    var value: Long = 0
  ...
  def getHighRiskUsers(filter: String, customAttribute: String): JSONArray = {
    val connection: Connection = Common.getConnection()
    var sql: String = """SELECT COUNT(1) FROM "Anomalies"""
    var query: PreparedStatement = connection.prepareStatement(sql)
    if (filter != "all")
    {
      sql = """SELECT "Review Status", "key" AS "Windows User Account", "1: files" AS "Document at Risk", "2: directory" AS "Folder at Risk" FROM "Anomalies" WHERE "1: files" IS NOT NULL AND "Review Status" LIKE ?"""
      query = connection.prepareStatement(sql)
      query.setString(1, filter)
    }
    else
    {
      sql = """SELECT "Review Status", "key" AS "Windows User Account", "1: files" AS "Document at Risk", "2: directory" AS "Folder at Risk" FROM "Anomalies" WHERE "1: files" IS NOT NULL"""
      query = connection.prepareStatement(sql)
    }
    val rst: ResultSet = query.executeQuery()
  ...
  def getHighRiskUsersCount(): Int = {
    val connection: Connection = Common.getConnection()
    val query: String = "select count(*) from \"Anomalies\" where \"1: files\" is not null"
  ...
  def getHighRiskFolderCount(): Int = {
    val connection: Connection = Common.getConnection()
    val query: String = "select \"2: directory\" from \"Anomalies\" where \"1: files\" is not null"
    val rst: ResultSet = connection.createStatement().executeQuery(query)
    var count: Int = 0
    while (rst.next())
    {
  ...
  def getHighRiskDocCount(): Int = {
    val connection: Connection = Common.getConnection()
    val query: String = "select \"1: files\" from \"Anomalies\" where \"1: files\" is not null"
    val rst: ResultSet = connection.createStatement().executeQuery(query)
    var count: Int = 0
    while (rst.next())
    {
  ...
  def getAllGUID(user: String): List[String] = {
    val connection: Connection = Common.getConnection()
    val sql: String = "select DISTINCT(\"objectGUID\") from \"USERS\""
    val query: PreparedStatement = connection.prepareStatement(sql)
    query.setString(1, user)
    val rst: ResultSet = query.executeQuery()
  ...
  def getName(user: String): String = {
    val connection: Connection = Common.getConnection()
    val sql: String = "select \"key\" from \"USERS\" where \"objectGUID\" = ?"
    val query: PreparedStatement = connection.prepareStatement(sql)
    query.setString(1, user)
    val rst: ResultSet = query.executeQuery()
    var name: String = ""
  ...
  def getClientDataRepoSecondTable(): JSONArray = {
    val connection: Connection = Common.getConnection()
    val sql: String = "select TO_NUMBER(\"TotalAnomaly\") as \"Total Anomaly\",TO_NUMBER(\"TotalDocumentsAtRisk\") as \"Document at Risk\",TO_NUMBER(\"TotalActiveDirectoryAtRisk\") as \"Active Directory at Risk\"  from \"ManageAnomaly\" where \"key\" = 'client_anomaly'"
    val query: PreparedStatement = connection.prepareStatement(sql)
    val rst: ResultSet = query.executeQuery()
  ...
  def getClientDataHighRiskUsers(filter: String): JSONArray = {
    ...
    if (filter != "all")
    {
      sql = "select \"owner\" as \"Windows User Account\",TO_NUMBER(\"DocumentsAtRisk\") as \"Document at Risk\",\"Confidentiality\",TO_NUMBER(\"FolderAtRisk\") as \"Folder at Risk\",\"SecurityInFault\" as \"Security in Fault\",\"ReviewStatus\" as \"Review Status\" from \"ManageAnomaly\" where \"key\" like 'summary_client_anomaly_%' and \"ReviewStatus\" like ?"
      query = connection.prepareStatement(sql)
      query.setString(1, filter)
    }
    else
    {
      sql = "select \"owner\" as \"Windows User Account\",TO_NUMBER(\"DocumentsAtRisk\") as \"Document at Risk\",\"Confidentiality\",TO_NUMBER(\"FolderAtRisk\") as \"Folder at Risk\",\"SecurityInFault\" as \"Security in Fault\",\"ReviewStatus\" as \"Review Status\" from \"ManageAnomaly\" where \"key\" like 'summary_client_anomaly_%'"
      query = connection.prepareStatement(sql)
    }
    val rst: ResultSet = query.executeQuery()
    val response: JSONArray = convertResultSetIntoJSON(rst, "")
